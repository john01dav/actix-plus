...
use jsonwebtoken::{encode, DecodingKey, EncodingKey, Header, decode, Validation};
...
impl<DataProviderImpl: DataProvider> AuthenticationProvider<DataProviderImpl> {
    ...
    pub fn register(
        ...
        if !EMAIL_REGEX.is_match(account.email()) {
            return Ok(RegistrationOutcome::InvalidEmail);
        }

        //check for existing account with same username
        if let Some(_) = self.provider.fetch_account(account.email())? {
    ...
    pub fn login(&self, email: &str, password: &str) -> ResponseResult<LoginOutcome> {
        //get account & verify exists
        let (account, hash_string) = match self.provider.fetch_account(email)? {
        ...
        Ok(LoginOutcome::Successful(cookie))
    }
    ...
    pub fn current_user(&self, request: &HttpRequest) -> ResponseResult<DataProviderImpl::AccountType> {
        ...
        //check token
        let token = match decode::<JsonWebTokenClaims<DataProviderImpl::AccountType>>(&cookie.value(), &DecodingKey::from_secret(&self.jwt_secret), &Validation::default()) {
            Ok(token) => token,
            Err(_) => return Err(ResponseError::StatusCodeError {
                message: "Unauthorized".into(),
                code: StatusCode::UNAUTHORIZED,
            })
        };
